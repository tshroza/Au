...
Given(/^user '(.*?)' is logged in/, (user: string) => { 
  if (user === 'base') {
    Helper.navigateToApp('/');
    Helper.fillInputFieldById('username', environment.user_base_name);
    Helper.clickButtonById('nextButton');
    Helper.fillInputFieldById('password', environment.user_base_password);
    Helper.clickButtonById('loginButton');
    return Helper.waitForVisibilityByCss('gyr-actions-widget');
  } else {
...
Given(/user navigated to accounts '(.*?)'/, (url: string) => {
  browser.waitForAngularEnabled(false);
  return browser.get(environment.loginUrl + url);
});
